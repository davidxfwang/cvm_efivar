...
static char default_efivarfs_path[] = "/sys/firmware/efi/efivars/";

static char *
find_efivar_filename(char *name, char *guid_name)
{
	DIR *d;
	struct dirent *dir;
	char *path;
	...
	if ( d ) {
		while ( (dir = readdir(d)) != NULL ) {
			if ( strstr(dir->d_name, name) ) {
				ptr = strchr(dir->d_name, '-');
				ptr++;
				sprintf(guid_name, "%s-%s", ptr, name);
				closedir(d);
				return name;
			}
		}
...
static void
prepare_data_cli(char *optarg, void **data, size_t *data_size, char *name)
{
	if ( !strcmp(name, "CvmTurbo") ) {
		int turbo_mode;

		turbo_mode = atoi(optarg);
		if ( turbo_mode > 3 ) {
			printf("Invalid turnbo mode\n");
			goto err;
		} else {
			char *mode_value = malloc(4);
	
			mode_value[0] = turbo_mode & 0xff;
			mode_value[1] = 0;
			mode_value[2] = 0;
			mode_value[3] = 0;
			*data = (void *)mode_value;
			*data_size = 4;
		}
	} else if ( !strcmp(name, "CvmCppcCtl") ) {
		char *magic = malloc(4);
		...
		if ( !strcmp(optarg, "enable") ) {
			...
			printf("Invalid action on CppcCtl\n");
			goto err;
		}
		*data = (void *)magic;
		*data_size = 4;
	} else if ( !strcmp(name, "CvmHyperThread") ) {
		int thread;

		thread = atoi(optarg);
		if ( (thread != 0) && (thread != 1) && (thread != 2) && (thread != 4) ) {
			printf("Invalid Hyper thread number\n");
			goto err;
		} else {
			char *thread_value = malloc(4);
	
			thread_value[0] = thread & 0xff;
			thread_value[1] = 0;
			thread_value[2] = 0;
			thread_value[3] = 0;
			*data = (void *)thread_value;
			*data_size = 4;
		}
	} else {
		printf("Variable is not supported with option -N, please use -n instead.\n");
		return;
	}
...
usage(const char *progname)
{
	...
	printf("  -N, --name=<name>                 variable to manipulate, no guid, in the form Boot0000\n");
	printf("  -a, --append                      append to variable specified by --name\n");
	printf("  -f, --fromfile=<file>             use data from <file>\n");
	printf("  -s, --fromstring=<hex string>     use data from <string>\n");
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, sopts, lopts, &i)) != -1) {
		switch (c) {
			...
			case 'N':
				{
				unsigned int guid_len = sizeof("a9f76944-9749-11e7-96d5-736f2e5d4e7e");
				char *guid_buf = malloc(guid_len + strlen(optarg) + 2);
				cli_name = find_efivar_filename(optarg, guid_buf);
				if ( guid_buf[0] == 0 ) {
					return EXIT_FAILURE;
				}
				name = guid_buf;
			...
			case 'f':
				...
				break;
			case 's':
				prepare_data_cli(optarg, &data, &data_size, cli_name);
				break;
			case 't':
